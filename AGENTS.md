...
- **Verify with a real render before reporting done, visual or behavioral.** Use the `browser-verify` skill for this — it covers everything from a single "does this render right" screenshot to a full auth-crossing, data-mutating flow, and its own quick-start routes you to whichever weight the check actually needs (a one-line screenshot command needs none of its scripts or references; a real workflow does). Always headless — never `--headed`, never launch a visible browser. The Playwright MCP server is retired; do not use `mcp__playwright__*` tools. Reuse the project's already-running dev server rather than starting a second one, and leave it running when you finish (the developer iterates repeatedly). Check at least one desktop and one narrow width for responsive work, and exercise the awkward states (a list scrolled with many items, the empty state, the signed-out variant) not just the happy path. When a change is a color/style override competing with an existing rule (a shadcn primitive default, a theme token, cascade/specificity), a screenshot glance is not proof it worked — confirm the override actually took effect by reading the element's computed styles. Save screenshots to `.playwright/<name>.png` (gitignored); a bare filename lands in the repo root.
