...
- **Dynamic reads need a Suspense boundary low enough to leave a real shell (Cache Components).** Reading `searchParams`, `cookies()`, `headers()`, or the session (`getClaims`, `getUser`) makes a component dynamic, and under Cache Components it must render inside a `<Suspense>` boundary or the build fails with _"Uncached data was accessed outside of `<Suspense>`."_ A route `loading.tsx` satisfies the build but **not** the Next 16.3 instant-navigation check, which flags a boundary that hides the whole page, static headings included, behind one fallback. So never `await` a dynamic read at the top of a page: move it into a small async child inside its own `<Suspense>` with a fallback shaped like the content, and keep everything user-independent in the shell (`/browse`, `/dashboard/new`, `/dashboard`). Only when every branch depends on per-user data and no user-independent shell exists, export `instant = false` instead (`/dashboard/edit/[id]`). A segment with no `loading.tsx` and no ancestor boundary (`(auth)/`) needs the explicit `<Suspense>` either way; since a component cannot sit inside a boundary it renders itself, put the `await` in a small async child of that boundary.
